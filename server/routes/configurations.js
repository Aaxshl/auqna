...
router.get('/', async (req, res) => {
  try {
    ...
    // Build query - if adminRole is provided, get role-specific config, otherwise get global
    const query = adminRole
      ? { configType: 'autoExport', adminRole }
      : { configType: 'autoExport', adminRole: 'global' };

    const config = await Configuration.findOne(query);

    // Default auto-export config
    ...
    res.json({
      autoExport: config?.autoExport || defaultAutoExport,
